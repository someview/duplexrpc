...
type HashRing struct {
	...
	replicas int
	keys     []int // Sorted
	hashMap  map[int]*discovery.ServiceInstance
}
...
func NewHashRing(replicas int, fn Hash) *HashRing {
	m := &HashRing{
		replicas: replicas,
		hash:     fn,
		hashMap:  make(map[int]*discovery.ServiceInstance),
	}
	if m.hash == nil {
...
func (m *HashRing) Add(ins ...discovery.ServiceInstance) {
	for _, key := range ins {
...
func (m *HashRing) Get(key string) *discovery.ServiceInstance {
	if m.IsEmpty() {
		return nil
	}
